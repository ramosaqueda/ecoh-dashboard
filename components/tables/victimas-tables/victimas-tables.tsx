...
export function VictimasDataTable<TData extends Victima, TValue>({
  ...
  const [selectedVictima, setSelectedVictima] = useState<Victima | null>(
    null
  );
  ...
  const handleUpdateCausa = async (causaId: number, data: any) => {
    try {
      setIsUpdating(true);
      const response = await fetch(`/api/causas-victimas/${causaId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (selectedVictima) {
        const updatedVictima = await queryClient.fetchQuery({
          queryKey: ['imputado', selectedVictima.id],
          queryFn: async () => {
            const response = await fetch(
              `/api/imputado/${selectedVictima.id}`
            );
            if (!response.ok) throw new Error('Error al actualizar víctima');
            return response.json();
          }
        });

        setSelectedVictima(updatedVictima);
      }
    } catch (error) {
  ...
  const table = useReactTable({
    ...
    meta: {
      onEdit,
      onDelete,
      onViewCausas: (victima: Victima) => setSelectedVictima(victima),
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} victima(s) encontrada(s)
        </div>
      </div>
      ...
      {selectedVictima && (
        <CausasDrawer
          ...
          onClose={() => setSelectedVictima(null)}
          causas={selectedVictima.causas}
          nombreSujeto={selectedVictima.nombreVictima}
